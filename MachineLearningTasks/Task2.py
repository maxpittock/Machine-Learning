...
def cluster_sort(dataset, k, centroid):
    ...
    for index in df_matrix:  
        ...
        #print(dataset_len, "leng")
        
        #create a matrix to store distances
        centroid_distance_matrix = np.ones(centroid_len)
        datase = np.ones(dataset_len)
        #All_Distances = np.ones(dataset_len ,centroid_len)

        #print(centroid_len, "len", centroid.shape)
...
def kmeans(dataset, k):
    ...
    while matching != True:
        ...
        df_with_clusters = cluster_sort(dataset, k, centroid)
...
def main():
    ...
    centroid, df_with_clusters = kmeans(df, 3)
